...
import com.bosspvp.api.items.advanced.args.LookupArgParser;
import com.bosspvp.api.items.advanced.provider.ItemProvider;
import com.bosspvp.api.items.advanced.types.*;
...
public class Items {
    ...
    private static TestableItem doParse(@NotNull final String[] args) {
        ...
        if (split.length == 2) {
            ...
            NamespacedKey namespacedKey = new NamespacedKey(namespace, keyID);
    ...
    public static CustomItem getOrWrap(@NotNull final TestableItem item) {
        if (item instanceof CustomItem) {
            ...
            return new CustomItem(
                    new NamespacedKey(BossAPI.getInstance().getCorePlugin(),
                            "wrapped_" + MathUtils.randInt(0, 100000)
                    ),
                    item::matches,
                    item.getItem()
            );
